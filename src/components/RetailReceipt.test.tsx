...
const file = new File(['%PDF-1.4'], 'CCM-Retail-07-09-2026-阿明.pdf', { type: 'application/pdf' })
...
async function ready() {
  ...
  await waitFor(() => expect(screen.getByRole('button', { name: 'PDF / 分享' })).toBeEnabled())
}
beforeEach(() => {
...
describe('Retail invoice output', () => {
  it('shares the prepared PDF File synchronously in the click and restores controls after completion', async () => {
    ...
    await ready()
    fireEvent.click(screen.getByRole('button', { name: 'PDF / 分享' }))
    // No awaiting generation, timeout or popup before this assertion.
    expect(canShare).toHaveBeenCalledWith({ files: [file] })
    expect(share).toHaveBeenCalledWith({ files: [file], title: 'CCM 门市现金结算单 · 阿明' })
    expect(screen.getByRole('status')).toHaveTextContent('正在分享 PDF')
    expect(screen.getByRole('button', { name: 'PDF / 分享' })).toBeDisabled()
    expect(createUrl).not.toHaveBeenCalled()
    await act(async () => complete())
    expect(screen.getByRole('button', { name: 'PDF / 分享' })).toBeEnabled()
  })
  it.each(['canShare false', 'share missing', 'canShare missing'])('shows explicit fallback when %s', async mode => {
    ...
    await ready(); fireEvent.click(screen.getByRole('button', { name: 'PDF / 分享' }))
    expect(screen.getByRole('button', { name: '打开 PDF' })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: '下载 PDF' })).toHaveAttribute('download', file.name)
  ...
  it('does not report an error when the user cancels the Share Sheet', async () => {
    ...
    expect(screen.queryByRole('button', { name: '打开 PDF' })).not.toBeInTheDocument()
  })
  it('also lets file-sharing browsers explicitly open or download their PDF', async () => {
    await ready(); fireEvent.click(screen.getByRole('button', { name: '其他 PDF 选项' }))
    expect(screen.getByRole('button', { name: '打开 PDF' })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: '下载 PDF' })).toHaveAttribute('href', 'blob:receipt')
    expect(share).not.toHaveBeenCalled()
  })
  ...
  it('reports print exceptions on the page', async () => {
    ...
    await ready(); fireEvent.click(screen.getByRole('button', { name: '打印' }))
  ...
  it('does not share a stale PDF when the invoice changes during preparation', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'PDF / 分享' }))
    expect(share).toHaveBeenCalledWith({ files: [nextFile], title: 'CCM 门市现金结算单 · 阿华' })
    await act(async () => {})
  })
  ...
  it('renders only stored invoice values and excludes controls from print layout', async () => {
    ...
    expect(screen.getByRole('button', { name: '打印' }).closest('.retail-no-print')).not.toBeNull()
