...
export function RetailReceipt({ sale }: { sale: RetailSale }) {
  return <section className="retail-receipt" aria-label="现金结算单"><h2>CCM Fishery 门市现金结算单</h2><p>日期：{sale.businessDate}</p><p>小贩：{sale.vendorName}</p>
    ...
    <table><colgroup><col className="retail-receipt-name" /><col className="retail-receipt-weight" /><col className="retail-receipt-price" /><col className="retail-receipt-amount" /></colgroup><thead><tr><th scope="col">品名</th><th scope="col">kg</th><th scope="col">RM/kg</th><th scope="col">金额 RM</th></tr></thead><tbody>{sale.lines.map((line, index) => <tr key={index}><td>{line.chineseName}<small>{line.malayName}</small></td><td>{retailWeightKg(line)}</td><td>{(line.unitPriceCents / 100).toFixed(2)}</td><td>{(line.amountCents / 100).toFixed(2)}</td></tr>)}</tbody></table>
...
export function RetailReceiptActions({ sale }: { sale: RetailSale }) {
  ...
  useEffect(() => {
    ...
    void createRetailInvoicePdf(sale).then(file => {
      ...
      setPrepared(result)
    }).catch(() => { if (current) setGenerationError('PDF 生成失败，请重试。') })
    return () => {
      current = false
  ...
  function print() {
    ...
    try {
      window.print()
      setNotice('已请求系统打印。如未出现打印界面，请在 Safari 中打开本页后重试。')
    } catch { setActionError('无法打开系统打印界面，请在 Safari 中重新打开本页后重试。') }
  }
  ...
  function showFallback(reason: string) {
    ...
    } catch { setActionError('无法准备 PDF 打开或下载链接，请刷新页面后重试。') }
  }

  async function share() {
    setActionError(''); setNotice('')
    if (!pdf) { setActionError('PDF 尚未准备好，请稍候重试。'); return }
    if (sharing) return
    try {
      if (!navigator.share || !navigator.canShare || !navigator.canShare({ files: [pdf.file] })) {
        showFallback(window.isSecureContext === false ? '当前连接不支持文件分享，请使用 HTTPS，或打开 / 下载 PDF。' : '此浏览器不支持 PDF 文件分享，请打开或下载 PDF。')
        return
      }
      // Invoke before the first await: the prepared File retains the direct click gesture.
      const request = navigator.share({ files: [pdf.file], title: `CCM 门市现金结算单 · ${sale.vendorName}` })
      setSharing(true)
      await request
    } catch (problem) {
      if (!(problem && typeof problem === 'object' && 'name' in problem && problem.name === 'AbortError')) {
        setActionError('PDF 分享失败，请重试，或使用打开 / 下载 PDF。')
        showFallback('可改用下方按钮打开或下载 PDF。')
      }
    } finally { setSharing(false) }
  }

  function openPdf() {
    if (!pdf?.url) { setActionError('PDF 尚未准备好，请重试。'); return }
    setActionError('')
    try {
      // Open synchronously from this click; no async generation or popup print path.
      const viewer = window.open('', '_blank')
      if (!viewer) { setActionError('PDF 打开被浏览器拦截，请允许此网站弹出窗口，或使用下载 PDF。'); return }
      viewer.opener = null
      viewer.location.replace(pdf.url)
      pdf.handedOff = true
      setNotice('已打开 PDF 预览。如新页面未显示内容，请返回并使用下载 PDF。')
    } catch { setActionError('无法打开 PDF，请使用下载 PDF，或在 Safari 中重试。') }
  }

  return <div className="retail-no-print retail-output-actions">
    <div className="retail-actions"><button type="button" className="primary-action" onClick={print}>打印</button>
      <button type="button" className="primary-action" disabled={!pdf || sharing} onClick={() => void share()}>PDF / 分享</button></div>
    {!pdf && !generationError && <p role="status">正在准备 PDF…</p>}
    {sharing && <p role="status">正在分享 PDF…</p>}
    {generationError && <><p className="error" role="alert">{generationError}</p><button type="button" onClick={() => setRetry(value => value + 1)}>重新生成 PDF</button></>}
    {actionError && <p className="error" role="alert">{actionError}</p>}
    {notice && <p role="status">{notice}</p>}
    {pdf && !fallback && <button type="button" disabled={sharing} onClick={() => showFallback('可打开 PDF 预览或下载文件。')}>其他 PDF 选项</button>}
    {fallback && pdf?.url && <div className="retail-pdf-fallback"><button type="button" onClick={openPdf}>打开 PDF</button>
      <a href={pdf.url} download={pdf.file.name} onClick={() => { pdf.handedOff = true; setNotice('已请求下载 PDF，请查看浏览器下载列表。') }}>下载 PDF</a></div>}
  </div>
}
