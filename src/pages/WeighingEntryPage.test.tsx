...
describe('iPhone 现场称重单页',()=>{
  ...
  it('支持 Enter 确认，并让鱼仔总重保留品质及 remark 而不增加篮数',async()=>{
    ...
    await waitFor(()=>expect(screen.getByLabelText('重量（kg）')).not.toBeDisabled())
    const bagQualityButton=screen.getByRole('button',{name:'包鱼仔'})
    fireEvent.click(bagQualityButton)
    await waitFor(()=>expect(bagQualityButton).toHaveAttribute('aria-pressed','true'))
    const totalModeButton=screen.getByRole('button',{name:'总重量'})
    fireEvent.click(totalModeButton)
    await waitFor(()=>expect(totalModeButton).toHaveAttribute('aria-pressed','true'))
