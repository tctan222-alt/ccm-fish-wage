...
describe('compact Retail history periods', () => {
  ...
  it('shows the full Monday–Sunday week and steps across a month boundary', () => {
    ...
    expect(screen.getByText('07092026 星期一 Mon – 13092026 星期日 Sun')).toBeInTheDocument()
    ...
    expect(screen.getByText('31082026 星期一 Mon – 06092026 星期日 Sun')).toBeInTheDocument()
  ...
  it('renders one compact clickable row per invoice with full accessible date, vendor and amount', () => {
    ...
    const row = screen.getByRole('link', { name: '查看结算单 View Invoice 07092026 星期一 Mon Ah Seng RM246.00' })
    expect(row).toHaveAttribute('href', '/retail-sales/history/second')
    expect(row).toHaveTextContent('07092026 星期一 Mon')
