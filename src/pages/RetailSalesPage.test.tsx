...
describe('mobile retail checkout', () => {
  it('shows Chinese, Malay and suggested price while typing and selects by keyboard without adding a line', () => {
    mount()
    const input = screen.getByRole('combobox', { name: '鱼名' })
    ...
    expect(screen.getByLabelText('实际 RM/kg')).toHaveValue('')
    ...
    expect(screen.getByLabelText('重量 kg')).toHaveFocus()
    expect(screen.getByRole('heading', { name: '本单明细（0）' })).toBeInTheDocument()
    fill('鱼名', 'MABONG')
    expect(screen.getByRole('option', { name: '马丰 mabong RM8.00/kg' })).toBeInTheDocument()
    expect(screen.getByLabelText('实际 RM/kg')).toHaveValue('')
    expect(screen.getByRole('button', { name: '加入明细' })).toBeDisabled()
  })
  it('creates inline, preserves the sale, and uses the saved fish before its subscription update', async () => {
    ...
    mount(); fill('日期', '2026-09-05'); fill('小贩名', '阿明'); add()
    expect(screen.getByRole('combobox', { name: '鱼名' })).toHaveFocus()
    fill('鱼名', '红鱼'); fireEvent.click(screen.getByRole('button', { name: '新增「红鱼」' }))
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument()
    expect(screen.getByLabelText('新鱼马来文名')).toHaveFocus()
    fill('新鱼马来文名', 'merah'); fill('新鱼建议 RM/kg', '12')
    fireEvent.click(screen.getByRole('button', { name: '保存并使用新鱼' }))
    ...
    expect(screen.getByLabelText('重量 kg')).toHaveFocus()
    expect(screen.getByLabelText('实际 RM/kg')).toHaveValue('12.00')
    expect(screen.getByLabelText('日期')).toHaveValue('2026-09-05')
    expect(screen.getByLabelText('小贩名')).toHaveValue('阿明')
    expect(screen.getByRole('heading', { name: '本单明细（1）' })).toBeInTheDocument()
    fill('重量 kg', '12.5'); fill('实际 RM/kg', '11.50')
    fireEvent.click(screen.getByRole('button', { name: '加入明细' }))
    expect(screen.getByRole('combobox', { name: '鱼名' })).toHaveFocus()
    fill('鱼名', 'merah')
    expect(screen.getByRole('option', { name: '红鱼 merah RM12.00/kg' })).toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: '清除当前品名' }))
  ...
  it('starts from an empty master and requires Malay and suggested price before using a new fish', async () => {
    ...
    mount(); fill('鱼名', '红鱼'); fireEvent.click(screen.getByRole('button', { name: '新增「红鱼」' }))
    expect(screen.getByLabelText('新鱼马来文名')).toBeRequired()
    expect(screen.getByLabelText('新鱼建议 RM/kg')).toBeRequired()
    expect(screen.getByRole('button', { name: '加入明细' })).toBeDisabled()
    fill('新鱼马来文名', '   '); fill('新鱼建议 RM/kg', '12')
    fireEvent.click(screen.getByRole('button', { name: '保存并使用新鱼' }))
    expect(await screen.findByRole('alert')).toHaveTextContent('马来文名')
    fill('新鱼马来文名', 'merah'); fill('新鱼建议 RM/kg', '0')
    fireEvent.click(screen.getByRole('button', { name: '保存并使用新鱼' }))
    expect(screen.getByRole('alert')).toHaveTextContent('售价')
    expect(services.quickAdd).not.toHaveBeenCalled()
    fill('新鱼建议 RM/kg', '12'); fireEvent.click(screen.getByRole('button', { name: '保存并使用新鱼' }))
    await waitFor(() => expect(screen.getByLabelText('实际 RM/kg')).toHaveValue('12.00'))
    fill('重量 kg', '7.3'); fireEvent.click(screen.getByRole('button', { name: '加入明细' }))
  ...
  it('preserves failed quick-add values and existing lines, and clears or cancels without creating a sale', async () => {
    ...
    mount(); fill('小贩名', '阿明'); add(); fill('鱼名', '红鱼')
    fireEvent.click(screen.getByRole('button', { name: '结算' }))
    expect(screen.getByRole('alert')).toHaveTextContent('请先加入或清除')
    fireEvent.click(screen.getByRole('button', { name: '新增「红鱼」' }))
    fill('新鱼马来文名', 'merah'); fill('新鱼建议 RM/kg', '12')
    fireEvent.click(screen.getByRole('button', { name: '保存并使用新鱼' }))
    await screen.findByText('网络中断，请重试')
    expect(screen.getByLabelText('新鱼马来文名')).toHaveValue('merah')
    expect(screen.getByLabelText('新鱼建议 RM/kg')).toHaveValue('12')
    expect(screen.getByRole('heading', { name: '本单明细（1）' })).toBeInTheDocument()
    expect(screen.getByLabelText('小贩名')).toHaveValue('阿明')
    expect(screen.getByRole('button', { name: '加入明细' })).toBeDisabled()
    ...
    fireEvent.click(screen.getByRole('button', { name: '新增「红鱼」' }))
    fireEvent.click(screen.getByRole('button', { name: '清除当前品名' }))
    expect(screen.queryByLabelText('新增门市鱼种')).not.toBeInTheDocument()
    expect(screen.getByRole('combobox', { name: '鱼名' })).toHaveValue('')
    expect(screen.getByRole('combobox', { name: '鱼名' })).toHaveFocus()
  ...
  it('accepts decimal kg, shows exact draft amounts, and preserves weights on the saved printable receipt', async () => {
    ...
    fireEvent.click(screen.getByRole('option', { name: /甘丰/ })); fill('重量 kg', '12.5')
  ...
  it('searches names, hides inactive species, accepts a missing default, and removes draft lines', () => {
    mount(); expect(screen.queryByRole('option', { name: /停用鱼/ })).not.toBeInTheDocument()
    fill('鱼名', 'mabong'); expect(screen.queryByRole('option', { name: /甘丰/ })).not.toBeInTheDocument()
    fill('鱼名', ''); add('无建议鱼', '5', '1.01')
  ...
  it('does not silently omit the current unadded line', async () => {
    mount(); fill('小贩名', '阿明'); add()
    fill('鱼名', '马'); fireEvent.click(screen.getByRole('option', { name: /马丰/ })); fill('重量 kg', '3')
    fireEvent.click(screen.getByRole('button', { name: '结算' }))
