...
打开 <http://127.0.0.1:8787>。后续启动不需要再次复制 `.env.example`。进程环境变量优先于 `.env`，修改凭据后重启本地服务。缺少凭据时页面标明未配置；比较会保存 `NOT_CONFIGURED` 错误，adapter 不发出网络请求。
...
3. 默认选中 OpenAI 和腾讯；点击「开始比较」才将**同一份 WAV 字节**通过本地 backend 并行送到选定 provider。没有自动上传、自动重试或答案提示。
...
- 每家准确率分母为该 provider 所有已保存尝试（包括失败）。平均耗时包含失败请求；错误次数包含 provider / 重量解析错误。未配置凭据的失败也计入，因此正式测评前应保留或另存本地数据目录，使用新的空 `benchmark.json` 开始一批；不要混合不同环境/模型成绩。
